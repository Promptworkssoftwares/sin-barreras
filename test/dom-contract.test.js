...
test('versioned frontend assets are loaded by protected app', () => {
  assert.match(appHtml, /app-bootstrap\.js\?v=1\.5\.3/);
  assert.match(appHtml, /styles\.css\?v=1\.5\.3/);
  assert.match(javascript, /learn\.js\?v=1\.5\.3/);
});
...
test('Aprender, Práctica, Coach and Sound Lab share automatic silence voice capture', () => {
  ...
  assert.match(serviceWorker, /voice-turn\.js\?v=1\.5\.3/);
});
...
test('Three.js conversational AI background is bundled and reacts to real conversation states', () => {
  ...
  assert.match(serviceWorker, /ai-stage\.js\?v=1\.5\.3/);
});
...
test('browser audio is unlocked from a user gesture before asynchronous AI playback', () => {
  ...
  assert.match(serviceWorker, /audio-playback\.js\?v=1\.5\.3/);
