...
test('v1.6.4 release metadata is aligned across Node, PWA and Android', () => {
  assert.match(text('package.json'), /"version": "1\.6\.4"/);
  assert.match(text('public/sw.js'), /sin-barreras-v1\.6\.4/);
  assert.match(text('android/app/build.gradle'), /versionCode = 1604/);
  assert.match(text('android/app/build.gradle'), /versionName = '1\.6\.4'/);
});
